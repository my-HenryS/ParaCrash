...
void logger_init(int rank, int nprocs) {
    ...
    const char* trace_dir = getenv("TRACE_DIR");
    if (trace_dir == NULL) trace_dir="recorder-logs";
    if(RECORDER_REAL_CALL(access)  (trace_dir, F_OK) != -1) {
        RECORDER_REAL_CALL(remove) (trace_dir);
    }
    RECORDER_REAL_CALL(mkdir) (trace_dir, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);

    char logfile_name[256];
    char metafile_name[256];
    sprintf(logfile_name, "%s/%d.itf", trace_dir, rank);
    sprintf(metafile_name, "%s/%d.mt", trace_dir, rank);
    ...
    if (rank == 0) {
        char global_meta_path[256];
        sprintf(global_meta_path, "%s/recorder.mt", trace_dir);
        FILE* global_metafh = RECORDER_REAL_CALL(fopen) (global_meta_path, "wb");
        ...
        sprintf(version_path, "%s/VERSION", trace_dir);
        FILE* version_file = RECORDER_REAL_CALL(fopen) (version_path, "w");
